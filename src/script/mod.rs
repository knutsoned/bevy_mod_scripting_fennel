...
use crate::script::asset::ScriptAssetPlugin;
...
impl Plugin for ScriptPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(ScriptAssetPlugin); //.add_systems(Startup, setup);
        /*
    ...
    unsafe {
        world_cell
            ...
            .consume(|engine, args| {
                ...
                engine.register_value("*world*", world);
                engine
                    .register_fn("-op", op)
...
fn op_bang(world: &mut WorldHolder, ty: String, name: String) -> Option<EntityRef> {
    ...
    let entity = match ty.as_str() {
        ...
        "composite" =>
            world.spawn((name, TextureOp, TextureOpType::<TextureOpComposite>::default())),
        "noise" => world.spawn((name, TextureOp, TextureOpType::<TextureOpNoise>::default())),
        "window" =>
            world.spawn((name, ComponentOp, ComponentOpType::<ComponentOpWindow>::default())),
        _ => {
            return None;
        }
    };
