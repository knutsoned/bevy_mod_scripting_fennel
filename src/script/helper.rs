...
// FIXME will probably need this eventually or else remove
...
pub struct RustylineHelper {
    #[allow(dead_code)] // FIXME this will eventually get used or removed
        ...
        if let Some((bracket, mut pos)) = self.bracket.get() {
            ...
            if let Some((matching, idx)) = find_matching_bracket(&line_to_highlight, pos, bracket) {
                ...
                line_to_highlight.replace_range(
                    idx..=idx,
                    &format!("\x1b[1;34m{}\x1b[0m", matching as char)
                );
...
// FIXME just seeing if this works as a placeholder for now
impl Highlighter for RustylineHelper {
    fn highlight<'l>(&self, line: &'l str, _pos: usize) -> Cow<'l, str> {
        use Cow::*;
