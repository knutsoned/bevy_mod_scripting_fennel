...
#[derive(Resource)]
/// FennelScriptHost is derived from bevy_mod_scripting's LuaScriptHost.
...
pub struct FennelScriptHost<A: LuaArg> {
    lua: Mutex<Lua>,
    _ph: PhantomData<A>,
}

impl<A: LuaArg> Default for FennelScriptHost<A> {
    fn default() -> Self {
        Self {
            _ph: Default::default(),
...
// FIXME this is where to add the Fennel-specific stuff to the ScriptHost
// try it with LuaFile for ScriptAsset instead of making a custom asset
// otherwise figure out the "right way" to do custom assets

// TODO: ensure run_one_shot works properly. It probably doesn't with the event handler changes.
impl<A: LuaArg> ScriptHost for FennelScriptHost<A> {
    ...
    fn load_script(
    ...
    ) -> std::result::Result<Self::ScriptContext, ScriptError> {
        {
            ...
            /* FIXME do something about errors because map_err does not exist
                .map_err(|e| {
...
if loaded[name] then 
    ...
    fn handle_events<'a>(
        ...
        /* code from sepiascraped looks like this:
