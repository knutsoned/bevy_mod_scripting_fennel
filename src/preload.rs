...
impl Plugin for LoadScreenPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(PendingHandles::default()).add_plugins(DefaultPlugins);
        /*
            .add_state::<GameState>()
...
pub struct PendingHandles(Vec<UntypedHandle>);
...
pub fn start_loading(asset_server: Res<AssetServer>, mut library: ResMut<PendingHandles>) {
    library.append(&mut asset_server.load_folder("scripts/"));
}
